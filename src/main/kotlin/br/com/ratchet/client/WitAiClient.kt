...
import br.com.ratchet.client.model.WitAiEntitiesResponse
import br.com.ratchet.client.model.WitAiEntityRequest
import br.com.ratchet.configuration.feign.witai.configuration.WitaiClientConfiguration
...
@FeignClient(name = "wit.ai", url = "\${witai.api}", configuration = arrayOf(WitaiClientConfiguration::class))
interface WitAiClient {
    ...
    @GetMapping(path = arrayOf("/entities/{entityId}"))
    fun getEntities(@PathVariable("entityId") entityId: String,
                    @RequestHeader("Authorization") token: String): WitAiEntitiesResponse
}
